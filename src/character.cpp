...
Character::Character(const sf::Vector2u window_size) {
  ...
  sprite_.setScale({Config::Character::kSpriteScale, Config::Character::kSpriteScale});
...
void Character::Update(const float dt, const UserInput user_input) {
  switch (state_) {
    case CharacterState::kNormal: {
      const auto acceleration = user_input.direction * Config::Character::kAcceleration;
    ...
    case CharacterState::kDashing: {
      ...
      if (dash_timer_ >= dash_duration_) {
        dash_timer_ = 0;
        velocity_ = dash_direction_ * dash_speed_ * Config::Character::kPostDashSpeedMultiplier;
